...
class PageFactory():
	def __init__(self, device, pages, app_name, **kwargs):
		...
		self.page_visited = [self.current_page]
	...
	def goto(self, page_name, *args, **kwargs):
		if hasattr(self.current_page, "goto_{}".format(page_name)):
			page_method = getattr(self.current_page, "goto_{}".format(page_name))()
			self.page_visited.append(self.current_page.__class__.__name__)
			if page_name in self.page_objects:
				new_page = self.page_objects[page_name]
			else:
				new_page = self.pages[page_name]()
				self.page_objects[page_name] = new_page
